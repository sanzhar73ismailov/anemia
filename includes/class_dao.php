...
class Dao{
	...
	public function getDicValues($ref_id){
		$results = array();
		$query =  "SELECT t.value_id, t.value_name
		FROM dic_val t 
		where t.dic_list_id='" . $ref_id . "' OR t.value_id='-1' 
		order by t.value_id";
		try {
			$stmt = $this->pdo->query($query);
			foreach($stmt as $row) {
	...
	public function getPatient($id){
		...
		$query =  "SELECT
				  ...
				  p.social_status_id,
				  p.doctor,
				  p.diag_cancer_localization_id,
				  p.diag_cancer_estab_date,
				  p.diag_cancer_histotype,
				  p.diag_cancer_degree_malignancy_id,
				  p.diag_cancer_tnm_stage_t_id,
				  p.diag_cancer_tnm_stage_n_id,
				  p.diag_cancer_tnm_stage_m_id,
				  p.diag_cancer_clin_stage_id,
				  p.diag_cancer_ecog_status_id,
				  p.lab_hb,
				  p.lab_hb_date,
				  p.lab_erythrocytes,
				  p.lab_erythrocytes_date,
				  p.user,
				  p.insert_date
				FROM 
				  " . DB_PREFIX . "patient p
	...
	private function fromRowToPatient($row, $therapy_array = null){
		...
		$entity->investigation_id=$row['investigation_id'];
		...
		$entity->diag_cancer_localization_id=$row['diag_cancer_localization_id'];
		...
		$entity->lab_hb=$row['lab_hb'];
		$entity->lab_hb_date=$row['lab_hb_date'];
		$entity->lab_erythrocytes=$row['lab_erythrocytes'];
		$entity->lab_erythrocytes_date=$row['lab_erythrocytes_date'];
